...
public class GWMCratesCommand implements CommandCallable {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "gui": {
                if (!(source instanceof ConsoleSource)) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("HAVE_NOT_PERMISSION"));
                    return CommandResult.success();
                }
                try {
            ...
            case "save": {
                ...
                source.sendMessage(GWMCrates.getInstance().getLanguage().getText("SUCCESSFULLY_SAVED"));
                return CommandResult.success();
            }
            case "reload": {
                ...
                source.sendMessage(GWMCrates.getInstance().getLanguage().getText("SUCCESSFULLY_RELOADED"));
                return CommandResult.success();
            }
            case "open": {
                if (args.length != 2) {
                ...
                if (!optional_player.isPresent()) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("COMMAND_CAN_BE_EXECUTED_ONLY_BY_PLAYER"));
                    return CommandResult.success();
                }
                Player player = optional_player.get();
                Optional<Manager> optional_manager = CratesUtils.getManager(manager_id);
                if (!optional_manager.isPresent()) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("MANAGER_NOT_EXIST",
                ...
                if (!open_manager.canOpen(player, manager)) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("CAN_NOT_OPEN_MANAGER"));
                    return CommandResult.success();
                }
                Case caze = manager.getCase();
                Key key = manager.getKey();
                if (caze.get(player) < 1) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("HAVE_NOT_CASE"));
                    return CommandResult.success();
                }
                if (key.get(player) < 1) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("HAVE_NOT_KEY"));
                    return CommandResult.success();
                }
                caze.add(player, -1);
                key.add(player, -1);
            ...
            case "force": {
                ...
                if (args.length == 3) {
                    ...
                    if (!optional_target.isPresent()) {
                        source.sendMessage(GWMCrates.getInstance().getLanguage().getText("PLAYER_NOT_EXIST",
                ...
                if (!optional_target.isPresent() && optional_player.isPresent() &&
                    ...
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("PLAYER_CAN_NOT_OPEN_MANAGER",
                            new Pair<String, String>("%PLAYER%", optional_target.get().getName())));
                    return CommandResult.success();
                }
                if (!optional_target.isPresent()) {
                    open_manager.open(optional_player.get(), manager);
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("CRATE_FORCE_OPENED",
                            new Pair<String, String>("%MANAGER%", manager.getName())));
                } else {
                    Player target = optional_target.get();
                    open_manager.open(target, manager);
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("CRATE_FORCE_OPENED_FOR_PLAYER",
                            new Pair<String, String>("%MANAGER%", manager.getName()),
                    ...
                    if (GWMCrates.getInstance().getConfig().getNode("TELL_FORCE_CRATE_OPEN_INFO").getBoolean(true)) {
                        target.sendMessage(GWMCrates.getInstance().getLanguage().getText("CRATE_FORCE_OPENED_BY_PLAYER",
            ...
            case "preview": {
                ...
                if (!optional_preview.isPresent()) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("PREVIEW_NOT_AVAILABLE",
                            new Pair<String, String>("%MANAGER%", manager.getName())));
                    return CommandResult.success();
                }
            ...
            case "buy": {
                ...
                if (!optional_economy_service.isPresent()) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("ECONOMY_SERVICE_NOT_FOUND"));
                    return CommandResult.success();
                }
                ...
                if (!optional_player_account.isPresent()) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("ECONOMY_ACCOUNT_NOT_FOUND"));
                    return CommandResult.success();
                }
                ...
                switch (args[1].toLowerCase()) {
                    case "case": {
                        ...
                        if (!optional_price.isPresent()) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("CASE_NOT_FOR_SALE"));
                            return CommandResult.success();
                        }
                        ...
                        if (price.compareTo(money) > 0) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("NOT_ENOUGH_MONEY"));
                            return CommandResult.success();
                        }
                        player_account.withdraw(currency, price, GWMCrates.getInstance().getCause());
                    ...
                    case "key": {
                        ...
                        if (!optional_price.isPresent()) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("KEY_NOT_FOR_SALE"));
                            return CommandResult.success();
                        }
                    ...
                    case "drop": {
                        ...
                        if (!optional_drop.isPresent()) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("DROP_NOT_EXIST",
                        ...
                        if (!optional_price.isPresent()) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("DROP_NOT_FOR_SALE"));
                            return CommandResult.success();
                        }
                        BigDecimal price = optional_price.get().multiply(amount);
                        if (price.compareTo(money) > 0) {
            ...
            case "give": {
                ...
                switch (args[2].toLowerCase()) {
                    case "case": {
                        ...
                        source.sendMessage(GWMCrates.getInstance().getLanguage().getText("SUCCESSFULLY_GIVE_CASE",
                                new Pair<String, String>("%MANAGER%", manager.getName()),
                                new Pair<String, String>("%PLAYER%", target_name)));
                        if (GWMCrates.getInstance().getConfig().getNode("TELL_GIVE_INFO").getBoolean(true)) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("GET_CASE_BY_PLAYER",
                                    new Pair<String, String>("%MANAGER%", manager.getName()),
                    ...
                    case "key": {
                        ...
                        source.sendMessage(GWMCrates.getInstance().getLanguage().getText("SUCCESSFULLY_GIVE_KEY",
                                new Pair<String, String>("%MANAGER%", manager.getName()),
                                new Pair<String, String>("%PLAYER%", target_name)));
                        if (GWMCrates.getInstance().getConfig().getNode("TELL_GIVE_INFO").getBoolean(true)) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("GET_KEY_BY_PLAYER",
                                    new Pair<String, String>("%MANAGER%", manager.getName()),
                                    new Pair<String, String>("%PLAYER%", source.getName())));
                        }
                        return CommandResult.success();
                    ...
                    case "drop": {
                        ...
                        source.sendMessage(GWMCrates.getInstance().getLanguage().getText("SUCCESSFULLY_GIVE_DROP",
                                new Pair<String, String>("%DROP_ID%", drop_id),
                                new Pair<String, String>("%MANAGER%", manager.getName()),
                                new Pair<String, String>("%PLAYER%", target_name)));
                        if (GWMCrates.getInstance().getConfig().getNode("TELL_GIVE_INFO").getBoolean(true)) {
                            source.sendMessage(GWMCrates.getInstance().getLanguage().getText("GET_DROP_BY_PLAYER",
                                    new Pair<String, String>("%DROP_ID%", drop_id),
                                    new Pair<String, String>("%MANAGER%", manager.getName()),
                                    new Pair<String, String>("%PLAYER%", source.getName())));
                        }
            ...
            case "list": {
                ...
                while (has_next) {
                    ...
                    if (has_next = manager_iterator.hasNext()) {
                        message_builder.append(GWMCrates.getInstance().getLanguage().getText("MANAGER_LIST_FORMAT",
                                new Pair<String, String>("%MANAGER_ID%", next.getId()),
                                new Pair<String, String>("%MANAGER_NAME%", next.getName())));
                    } else {
                        message_builder.append(GWMCrates.getInstance().getLanguage().getText("LAST_MANAGER_LIST_FORMAT",
                                new Pair<String, String>("%MANAGER_ID%", next.getId()),
                                new Pair<String, String>("%MANAGER_NAME%", next.getName())));
                    }
                }
                if (GWMCrates.getInstance().getLanguage().exists("MANAGER_LIST_HEADER")) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("MANAGER_LIST_HEADER"));
                }
                source.sendMessage(message_builder.build());
                if (GWMCrates.getInstance().getLanguage().exists("MANAGER_LIST_FOOTER")) {
                    source.sendMessage(GWMCrates.getInstance().getLanguage().getText("MANAGER_LIST_FOOTER"));
                }
                return CommandResult.success();
            }
            case "info": {
                ...
                Optional<Manager> optional_manager = CratesUtils.getManager(manager_id);
                if (!optional_manager.isPresent()) {
                    source.sendMessages(GWMCrates.getInstance().getLanguage().getText("MANAGER_NOT_EXIST",
                            new Pair<String, String>("%MANAGER_ID%", manager_id)));
                    return CommandResult.success();
                }
                Manager manager = optional_manager.get();
                ...
                for (int i = 0; i < drop_list.size(); i++) {
                    ...
                    if (i != drop_list.size() - 1) {
                        drops_builder.append(GWMCrates.getInstance().getLanguage().getPhrase("DROP_LIST_FORMAT",
                                new Pair<>("%ID%", drop.getId().orElse("Unknown ID"))));
                    } else {
                        drops_builder.append(GWMCrates.getInstance().getLanguage().getPhrase("LAST_DROP_LIST_FORMAT",
                                new Pair<>("%ID%", drop.getId().orElse("Unknown ID"))));
                    }
                }
                source.sendMessages(GWMCrates.getInstance().getLanguage().getTextList("MANAGER_INFO_MESSAGE",
    ...
    private void sendHelp(CommandSource source) {
        source.sendMessages(GWMCrates.getInstance().getLanguage().getTextList("HELP_MESSAGE",
